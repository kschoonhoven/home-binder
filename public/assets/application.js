...
$(document).ready(function() {
	...
	/*
	 * method to determine if an element exists
	 */
	 ...
	 * for adding items
	 */   	
   	prepareDataForm = function(form){
   		$('#addFormWell').html(form).fadeIn(function(){
   			bindTypeahead();
			bindDatePicker();
   		});
   		
		$(".cancel_data_form").on("click", function(e){
			closeDataForm();
		});
   	};
   	
   	showModalForm = function(form){
   		$('#formModal').html(form).modal('show');
   		bindTypeahead();
		bindDatePicker();
   	};
	...
	closeDataForm = function(){
		$('.data_form').fadeOut(function(){
			$('#addFormWell').fadeOut(function(){
				$('#addFormWell').empty();
			});
		});
	};
	
	showMoreInfo = function(header, content){
		$('#moreInfoHeader').html(header);
		$('#moreInfoBody').html(content);
		$('#moreInfoModal').modal('show');
	};
	
	showTabContent = function(itemId, content, type){
		var container;
		
		if($("#item_" + itemId + "_" + type).doesExist()) {
			container 	= "#item_" + itemId + "_" + type;
		}
		else {
			container 	= "#item_" + itemId + "_extras";
		}

		var data 		= container + "_content";
		var close 		= container + "_close";
	...
	updateTabContent = function(itemId, content, type){
		var data;
		
		if($("#item_" + itemId + "_" + type + "_content").doesExist()) {
			data = "#item_" + itemId + "_" + type;
		}
		else {
			data = "#item_" + itemId + "_extras_content";
		}
	...
	bindDelete = function() {
		...
		$('.delete_image').bind('ajax:success', function() {  
		...
		$('.delete_doc').bind('ajax:success', function() {  
	 ...
	 * for editing items
	 */ 
   	prepareEditForm = function(controller, form){
   		$("#" + controller + "_" + itemID).fadeOut(function(){
   			//$("#" + controller + "_edit_" + itemID).hide(function(){
   				$("#" + controller+ "_edit_"+ itemID).html(form);
   				$("#" + controller + "_edit_" + itemID).fadeIn(function(){
   					bindEditCancel();
   					bindTypeahead();
   					bindDatePicker();
   				});
 	...
 	bindEditCancel = function() {
		$(".cancel_edit_form").bind("click", function(e){
			var id 		= $(this).attr("data-itemid");
			var cont 	= $(this).attr("data-controller");
			
   			$("#" + cont + "_edit_" + id).fadeOut(function(){
   				$("#" + cont + "_edit_" + id).empty();
   				$("#" + cont + "_" + id).fadeIn();
   			});
		});
	};	
	...
	bindEdit = function() {
		$(".edit_item_btn").bind("click", function(e){
 	...
 	/*
	 * methods and bindings
	 * for showing and hiding item details
	 */ 
	showDetails = function(controller, id){
		$("#" + controller + "_details_" + id).slideDown('slow');
		$("#" + controller + "_expander_" + id).hide();
		$("#" + controller + "_contractor_" + id).show();
	};
	
	hideDetails = function(controller, id){
		$("#" + controller + "_details_" + id).slideUp('slow');
		$("#" + controller + "_contractor_" + id).hide();
		$("#" + controller + "_expander_" + id).show();
	};
	
	showAllDetails = function(){
		// toggle the details
		$('div[id*="details"]').slideDown('slow');
		$('a[id*="contractor"]').show();
		$('a[id*="expander"]').hide();
		
		// toggle the toggle
		//$('#hideAllButton').show();
		//$('#showAllButton').hide();
		$("#hideShowAll").html("Collapse All");
		$("#hideShowAll").data("status", "expanded");
	};
	
	hideAllDetails = function(){
		// toggle the details
		$('div[id*="details"]').slideUp('slow');
		$('a[id*="contractor"]').hide();
		$('a[id*="expander"]').show();
		
		// toggle the toggle
		//$('#hideAllButton').hide();
		//$('#showAllButton').show();		
		$("#hideShowAll").html("Expand All");
		$("#hideShowAll").data("status", "collapsed");	
	};
	
	toggleAllDetails = function(){
		if ($("#hideShowAll").data("status") == 'expanded'){
			hideAllDetails();
		}
		else{
			showAllDetails();
	...
	updateDataTable = function(data, count){
		$("#dataTable").html(data);
		$("#item_count").html('<strong>' + count + '</strong>');
	...
	bindTypeahead = function(){
		$(".autocomplete").each(function(ta){
			var typeahead_locals = $(this).data("local");
			var typeahead_name = $(this).data("name");
			var span = $(this).data("span");
			$(this).typeahead({
				name: typeahead_name,
				local: typeahead_locals
			});
			// Need to apply the sizing after typeahead
			$(this).parent().addClass(span);
			$(this).prev().addClass("span12"); // The hint input
			$(this).addClass("span12").css("margin-left", "0px"); // The input
			// Set the width of the dropdown
			$(".tt-dropdown-menu").addClass("span12").css("margin-left","0px");
		});
	};

	bindDatePicker = function(){
		$("input.datepicker").each(function(dp){
			$(this).datepicker({
				changeMonth: true,
				changeYear: true,
	...
	initUploader = function(itemid, type){
		var filesrem = 0;
		var uploader = new plupload.Uploader({
			runtimes : 'html5,flash,silverlight',
			browse_button : 'pickfiles_'+type+'_'+itemid,
			container : 'container_'+type+'_'+itemid,
			drop_element : 'dropzone_'+type+'_'+itemid,
			max_file_size : '10mb',
			url: gon.url,
			...
			multipart: true,
	    	multipart_params:{
		      'key': gon.user + '/' + gon.binder + '/' + gon.controller + '/' + itemid + '/' + type + '/${filename}',
		      'Filename': '${filename}', 
		      'acl': gon.acl,
		      'Content-Type': 'multipart/mixed',
		      'success_action_status': '201',
		      'AWSAccessKeyId' : gon.access_key_id,
		      'policy': gon.policy,
		...
		$('#uploadfiles_'+type+'_'+itemid).click(function(e) {
			uploader.start();
			e.preventDefault();
		});
		...
		// when files are added, show them in the queue
		// show the upload button if it is not
		uploader.bind('FilesAdded', function(up, files) {
			...
			$.each(files, function(i, file) {
				$('#filelist_'+type+'_'+itemid).append(
					'<div class="upload-queue-file" id="' + file.id + '"><a href="#" id="deleteFile_' + file.id + '">&times;</a>' +
						file.name + ' (' + plupload.formatSize(file.size) + ') <b class="pull-right"></b>' +
					'</div>');
				
				$('#deleteFile_' + file.id).click(deleteHandle(up, file));
			});
			
			if($('#uploadfiles_'+type+'_'+itemid).is(':hidden')) {
				$('#uploadfiles_'+type+'_'+itemid).show();
			}
	
			up.refresh(); // Reposition Flash/Silverlight
		});
		
		$('#uploadfiles_'+type+'_'+itemid).bind("click", function(){
			$(this).addClass('disabled');
		});
		
		// keep track of the files remaining
		uploader.bind('QueueChanged', function(up, files){
	        filesrem = uploader.files.length;
	    });		
	
		// update percentage as file uploads
		uploader.bind('UploadProgress', function(up, file) {
			$('#' + file.id + " b").html(file.percent + "%");
		});
		
		// show the error if a file upload fails
		uploader.bind('Error', function(up, err) {
			$('#filelist_'+type+'_'+itemid).append("<div class='error'>Error: " + err.code +
				", Message: " + err.message +
		...
		// when an upload completes, style it
		uploader.bind('FileUploaded', function(up, file, info) {
			$('#' + file.id + " b").html("100%");
			$('#deleteFile_' + file.id).remove();
			
			var resp = jQuery.parseXML(info.response);
			$xml = $(resp);
			
			var key = $xml.find("Key").text();
			var keyValues = key.split("/");
			
			if (keyValues[4] == "image"){
				$.post(
					"/" + keyValues[2] + "/" + keyValues[3] + "/images.json",
					{
						image:{
							imageable_id: keyValues[3],
				...
				$('#item_' + keyValues[3] + '_images_content ul').append(
					"<li><a class='thumbnail' href='" + $xml.find("Location").text() + "'><img style='max-width: 200px;' src='" + $xml.find("Location").text() + "'></a></li>"
				);
			}
			else if (keyValues[4] == "doc"){
				$.post(
					"/" + keyValues[2] + "/" + keyValues[3] + "/documents.json",
					{
						"document":{
							documentable_id: keyValues[3],
							name:keyValues[keyValues.length-1],
							location:$xml.find("Location").text(),
							bucket:$xml.find("Bucket").text(),
							key:$xml.find("Key").text(),
							etag:$xml.find("ETag").text()
				...
				$('#item_' + keyValues[3] + '_docs_content').append(
					"<div id='doc_" + keyValues[3] + "' class='note'><a target='blank' href='" + $xml.find("Location").text() + "'>" + keyValues[keyValues.length-1] + "</a><div class='created_at'>" + month + "/" + day + "/" + year + "</div></div>"
				);
			}
			
			filesrem--;
	        if (filesrem == 0) {
	            $('#uploadfiles_'+type+'_'+itemid).hide();
	            $('#uploadfiles_'+type+'_'+itemid).removeClass('disabled');
	            
	            // clear the queue
	            up.splice();
	            $('#filelist_'+type+'_'+itemid).empty();
	        }
		});
	};
		
	bindUpload = function() {
		$('.upload-tab').bind("click", function(e){
			var itemid 	= $(this).attr("data-itemid");
			var type	= $(this).attr("data-uptype");
			
			initUploader(itemid, type);
		});
		
		$('.dropzone').bind("dragstart dragenter dragover drag", function(e){
			$(this).addClass("dropzone-big");
			$(this).addClass("dropzone-hover");
			$('.dropzone-text').show();
		});
			
		$('.dropzone').bind("dragleave drop dragend", function(e){
			$(this).removeClass("dropzone-big");
	...
	initControls = function() {
		...
		$(document).on('mouseenter', '.photo-gallery', function(){ 
			bindDelete();
			$('a[data-toggle="tooltip"]').tooltip();
		});
		
		$(document).on('mouseenter', '.doc-gallery', function(){ 
			bindDelete();
			$('a[data-toggle="tooltip"]').tooltip();
		});		
	};
...
function displayInShade(html){
	$('#shadeContent').html(html);
	$('#shade').css("left", ($('#app-header').width() - $('#shade').width()) / 2); 
	$('#shade').css("top", $('#app-header').height() + 1);
	$('#shade').css('z-index', '9999');
	$('#shade').slideDown();
	$("#closeShade").bind("click", function(){
		$("#shade").slideToggle();
	});
}
